...
void load_popularity(py::array_t<double>& tau_p)
{
	...
	if ((fp = fopen("./data/Amazon-Music/item_interactions.csv", "r")) != NULL)
	{ 
...
void load_user_interation_val()
{
	...
	if ((fp = fopen("./data/Amazon-Music/train_list.txt", "r")) != NULL)                                //*
	{
		while (!feof(fp))
		{
			line = fgets(buffer, sizeof(buffer), fp);
...
void load_user_interation_test()
{
	...
	int itr_num_user, item_id;
	// printf("loading user interaction\n");
	if ((fp = fopen("./data/Amazon-Music/train_list_t.txt", "r")) != NULL)                         //*
	{
		while (!feof(fp))
		{
			line = fgets(buffer, sizeof(buffer), fp);
